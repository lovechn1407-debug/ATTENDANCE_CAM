...
/**
 * Physical Liveness Pose & Expression Analysis (Smile, Turn Left, Turn Right)
 */
export function analyzeLivenessPose(detection) {
  if (!detection || !detection.landmarks) {
    return { isSmiling: false, isTurnedLeft: false, isTurnedRight: false };
  }
  ...
  if (detection.expressions && typeof detection.expressions.happy === "number") {
    if (detection.expressions.happy > 0.45) {
      isSmiling = true;
    }
  }
  ...
  if (!isSmiling && points && points.length >= 68) {
    ...
    if (mouthHeight > 0 && (mouthWidth / mouthHeight) > 2.4) {
      isSmiling = true;
    }
  }

  // 2. Head Turn Left / Right Detection using Nose Tip (pt 30) & Jaw (pt 0 & pt 16)
  ...
  if (points && points.length >= 68) {
    ...
    if (faceWidth > 0) {
      ...
      // In video stream:
      // noseRatio < 0.36 -> Turned Left
      // noseRatio > 0.64 -> Turned Right
      if (noseRatio < 0.36) {
        isTurnedLeft = true;
      } else if (noseRatio > 0.64) {
        isTurnedRight = true;
      }
    }
  }

  return { isSmiling, isTurnedLeft, isTurnedRight };
}
