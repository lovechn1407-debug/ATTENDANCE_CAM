  ...
  return onValue(studentsRef, (snapshot) => {
    if (snapshot.exists()) {
      ...
      const studentList = Object.keys(data).map((key) => {
        let parsedSub = [];
        if (Array.isArray(data[key].subjects)) {
          parsedSub = data[key].subjects;
        } else if (typeof data[key].subjects === "string") {
          parsedSub = data[key].subjects.split(";").flatMap(s => s.split(",")).map(s => s.trim()).filter(Boolean);
        }
...
export async function addStudent(studentData) {
  try {
    ...
    if (Array.isArray(studentData.subjects)) {
      parsedSub = studentData.subjects;
    } else if (typeof studentData.subjects === "string") {
      parsedSub = studentData.subjects.split(";").flatMap(s => s.split(",")).map(s => s.trim()).filter(Boolean);
    }
  ...
  try {
    ...
    studentsArray.forEach(student => {
      ...
      if (Array.isArray(student.subjects)) {
        parsedSub = student.subjects;
      } else if (typeof student.subjects === "string") {
        parsedSub = student.subjects.split(";").flatMap(s => s.split(",")).map(s => s.trim()).filter(Boolean);
      }
  ...
  return onValue(staffsRef, (snapshot) => {
    if (snapshot.exists()) {
      ...
      const list = Object.keys(data).map(key => ({
        ...
        allotments: Array.isArray(data[key].allotments) ? data[key].allotments : []
      }));
      callback(list);
    } else {
      callback([]);
...
export async function saveStaff(staffData) {
  try {
    ...
    if (Array.isArray(staffData.subjects)) {
      ...
      parsedSubjects = staffData.subjects.split(",").flatMap(s=>s.split(";")).map(s => s.trim()).filter(Boolean);
    }
    ...
    const payload = {
      ...
      allotments: Array.isArray(staffData.allotments) ? staffData.allotments : (existing.allotments || []),
  ...
  try {
    ...
    staffsArray.forEach(staff => {
      ...
      if (Array.isArray(staff.subjects)) {
        ...
        parsedSubjects = staff.subjects.split(",").flatMap(s=>s.split(";")).map(s => s.trim()).filter(Boolean);
      }
      ...
      updates[`staffs/${idToUse}`] = {
        ...
        subjects: parsedSubjects,
        allotments: Array.isArray(staff.allotments) ? staff.allotments : [],
        createdAt: timestamp,
        updatedAt: timestamp
      };
    });
...
/* ==========================================
   MOBILE QR PHONE CAMERA CAPTURE SERVICES
   ========================================== */

export async function createMobileCaptureSession(sessionId, studentInfo = {}) {
  try {
    const sessionRef = ref(db, `mobile_capture_sessions/${sessionId}`);
    const payload = {
      sessionId,
      status: "pending",
...
export function subscribeToMobileCaptureSession(sessionId, callback) {
  if (!sessionId) return () => {};
  const sessionRef = ref(db, `mobile_capture_sessions/${sessionId}`);
  return onValue(sessionRef, (snapshot) => {
    if (snapshot.exists()) {
    ...
    console.error("Error subscribing to mobile capture session:", error);
    callback(null);
  });
}

export async function submitMobileCaptureSession(sessionId, photos = []) {
  try {
    const sessionRef = ref(db, `mobile_capture_sessions/${sessionId}`);
    const payload = {
      status: "completed",
    ...
    console.error("Error submitting mobile capture session:", error);
    throw error;
  }
}

export async function deleteMobileCaptureSession(sessionId) {
  try {
    if (!sessionId) return;
    const sessionRef = ref(db, `mobile_capture_sessions/${sessionId}`);
...
/* ==========================================
   ACADEMIC STRUCTURE & SETTINGS SERVICES
   ========================================== */

export const DEFAULT_ACADEMIC_SETTINGS = {
  courses: ["B.Tech", "M.Tech", "BCA", "MCA", "B.Sc", "M.Sc", "MBA", "BBA"],
  departments: ["Computer Science", "Information Technology", "Electronics & Comm", "Mechanical Engineering", "Electrical Engineering", "Civil Engineering"],
  branches: ["CSE", "IT", "ECE", "ME", "EE", "CE", "AI/ML", "Data Science"],
  sections: ["A", "B", "C", "D", "1", "2"],
  subjects: ["Data Structures", "Operating Systems", "Computer Networks", "Database Systems", "Machine Learning", "Software Engineering", "Mathematics", "Physics"]
};

export function subscribeToAcademicSettings(callback) {
  const settingsRef = ref(db, "academicSettings");
  return onValue(settingsRef, (snapshot) => {
    if (snapshot.exists()) {
      const data = snapshot.val();
      callback({
        courses: Array.isArray(data.courses) ? data.courses : DEFAULT_ACADEMIC_SETTINGS.courses,
        departments: Array.isArray(data.departments) ? data.departments : DEFAULT_ACADEMIC_SETTINGS.departments,
        branches: Array.isArray(data.branches) ? data.branches : DEFAULT_ACADEMIC_SETTINGS.branches,
        sections: Array.isArray(data.sections) ? data.sections : DEFAULT_ACADEMIC_SETTINGS.sections,
        subjects: Array.isArray(data.subjects) ? data.subjects : DEFAULT_ACADEMIC_SETTINGS.subjects,
        updatedAt: data.updatedAt || null
      });
    } else {
      callback(DEFAULT_ACADEMIC_SETTINGS);
    }
  }, (error) => {
    console.error("Error subscribing to academic settings:", error);
    callback(DEFAULT_ACADEMIC_SETTINGS);
  });
}

export async function updateAcademicSettings(settingsPayload) {
  try {
    const settingsRef = ref(db, "academicSettings");
    const payload = {
      courses: Array.isArray(settingsPayload.courses) ? settingsPayload.courses : DEFAULT_ACADEMIC_SETTINGS.courses,
      departments: Array.isArray(settingsPayload.departments) ? settingsPayload.departments : DEFAULT_ACADEMIC_SETTINGS.departments,
      branches: Array.isArray(settingsPayload.branches) ? settingsPayload.branches : DEFAULT_ACADEMIC_SETTINGS.branches,
      sections: Array.isArray(settingsPayload.sections) ? settingsPayload.sections : DEFAULT_ACADEMIC_SETTINGS.sections,
      subjects: Array.isArray(settingsPayload.subjects) ? settingsPayload.subjects : DEFAULT_ACADEMIC_SETTINGS.subjects,
      updatedAt: new Date().toISOString()
    };
...
export async function resetAcademicSettings() {
  try {
    const settingsRef = ref(db, "academicSettings");
    ...
    console.error("Error resetting academic settings:", error);
    throw error;
  }
}
